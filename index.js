...
app.post('/getdiff', async (req, res) => {
    const { account } = req.body
    const mine_work = getBagDifficulty(account);
	 console.log(mine_work.difficulty)
	res.json({account: mine_work})
})
...
const getBagDifficulty = async function (account) {
    try {
        ...
        const bag = await getBag(mining_account, account, eos_rpc, aa_api);
        const params = getBagMiningParams(bag);
		console.log('ffff ' + params.difficulty);
